...
- (void)setItems:(NSArray<GCMarqueeModel *> *)items {
    ...
    if (_type == GCMarqueeDirectionTypeLTR) {
        CGFloat y = (self.bounds.size.height - tmpItem.bounds.size.height) * 0.5;
        _scrollView.frame = CGRectMake(-_scrollView.contentSize.width, y, _scrollView.contentSize.width, tmpItem.bounds.size.height);

    } else if (_type == GCMarqueeDirectionTypeRTL || _type == GCMarqueeDirectionTypeReverse) {
        CGFloat y = (self.bounds.size.height - tmpItem.bounds.size.height) * 0.5;
        _scrollView.frame = CGRectMake(self.bounds.size.width, y, self.bounds.size.width + _scrollView.contentSize.width, tmpItem.bounds.size.height);
