...
+ (NSData *)compressImageQuality:(UIImage *)image toSpecifyByte:(NSInteger)maxLength {
    CGFloat compression = 1;
    NSData *data = UIImageJPEGRepresentation(image, compression);
    if (data.length < maxLength) {
        NSLog(@"JPEG 压缩质量1 转换后 大小符合要求,返回-------------");
        return data;
    }
    CGFloat max = 1;
    CGFloat min = 0;
...
{
    NSData *imageData = [UIImage compressImageQuality:image toSpecifyByte:maxLength];
    
    NSLog(@"%@", [NSString stringWithFormat:@"质量优先图片压缩后的大小：%f kb", imageData.length / 1000.0]);
    
    UIImage *resultImage = [UIImage imageWithData:imageData];
    
    NSLog(@"%@", [NSString stringWithFormat:@"质量优先图片的压缩后尺寸：w：%f h：%f 图片大小 = %lu", resultImage.size.width, resultImage.size.height, (unsigned long)[UIImage imageCost:resultImage]]);
...
+ (NSUInteger)imageCost:(UIImage *)image {
    ...
    CGFloat height = CGImageGetHeight(ref);
    size_t bitesPerRow = CGImageGetBytesPerRow(ref);
    NSUInteger cost = height * bitesPerRow;
    cost = MAX(1, cost);
    return cost;
}
